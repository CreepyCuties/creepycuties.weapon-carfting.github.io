...
function reducer(state: GameState, action: Action): GameState {
  switch (action.type) {
    ...
    case "DECOMPOSE": {
      ...
      const returned = Object.entries(recipe.requires)
        .map(([i, q]) => `${i} x${q}`)
        .join(", ") || "(nothing)";
...
export default function WeaponCraftingNarrativePrototype() {
  ...
  const currentRecipes = state.location === "sarah" ? SARAH_RECIPES : state.location === "sam" ? SAM_RECIPES : [];
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <Card
              title="Locations"
              right={<Pill>{narrativeStage}</Pill>}
            >
                      ...
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                          ...
                          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                            <Button
                              disabled={!unlocked}
                              onClick={() => dispatch({ type: "FIGHT_BOSS", bossId: b.id })}
                            >
                              Fight
                            </Button>
                          </div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {state.log.map((e) => (
                  <div key={e.t} style={{ opacity: 0.92, lineHeight: 1.35 }}>
                    <span style={{ opacity: 0.6, marginRight: 10, fontSize: 12 }}>
